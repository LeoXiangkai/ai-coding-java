...
4. 默认不引入强制 hooks、CLI 门禁或长流程阻塞，只提供按需规则、验证矩阵和 Review 口径。
5. P0 规则用于阻断明显高风险交付；P1/P2 主要用于 Review 和交付报告，不把日常开发拖成重型流程。
...
- 模板更新必须保持轻量，不复制重型命令体系、hooks、多人评审门禁或全量流程，除非项目明确启用。
...
ai-coding-java 模板默认只提供轻量规则和报告格式，不默认安装阻断式 hooks。是否启用强门禁由项目负责人决定，并应在项目 `AGENTS.md` 或项目画像中明确。
