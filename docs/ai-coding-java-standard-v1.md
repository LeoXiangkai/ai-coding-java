...
5. 不引入未经确认的依赖。
6. 不写明文密钥。
7. 不把清楚的功能目标降级成临时脚本、局部兜底或后续再补；应完成对应的设计、实现、测试和验证入口。
...
ai-coding-java 模板默认会安装轻量 Git hooks。`pre-commit` 对确定性 P0 风险阻断提交；`pre-push` 在 warn 模式检查个人开发分支和验证命令配置，在 strict 模式执行 build/test。
